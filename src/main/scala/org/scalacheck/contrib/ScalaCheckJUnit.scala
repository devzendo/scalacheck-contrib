        ...
        properties.properties.map({ propTuple =>
            propTuple match {
                case (desc, prop) => {
                    ...
                    ScalaCheckTest.check(new Parameters.Default {
                        override val testCallback: AnyRef with Test.TestCallback = consoleReporter chain
                          new CustomTestCallback(notifier, descObj)
                    }, prop)
